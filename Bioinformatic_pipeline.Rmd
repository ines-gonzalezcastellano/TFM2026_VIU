---
title: "Análisis del microbioma de una especie de interés en conservación mediante DNA metabarcoding"
output:
...
```{bash, eval = FALSE}
mkdir "$BASE_DIR/01_QC"
cd "$BASE_DIR/01_QC"
fastqc -t 7 "../00_Raw_data/*.fastq.gz" -o .
```
...
```{bash, eval = FALSE}
mkdir "$BASE_DIR/01_QC/R1"
mkdir "$BASE_DIR/01_QC/R2"

mv *_R1* "$BASE_DIR/01_QC/R1"
mv *_R2* R2"$BASE_DIR/01_QC/R2"

cd "$BASE_DIR/01_QC/R1"
multiqc .
mv multiqc_report.html multiqc_report_R1.html

cd "$BASE_DIR/01_QC/R2"
multiqc .
...
```{r, eval = FALSE}
archivos <- list.files(
  file.path(base_dir, "01_QC", "FLASH"),
  pattern = "\\.hist$",
  full.names = TRUE
)
...
#legend("topright", legend = basename(archivos),
#       col = colores, lwd = 2, cex = 0.7)
```
...
```{bash, eval = FALSE}
mkdir "$BASE_DIR/02_METADATA"
cd "$BASE_DIR/02_METADATA"
...
**Se emplearán los resultados de este denoising para hacer la asignación taxonómica.**
...
Descargar la última versión de SILVA y formatearla. Seguir el manual de <https://forum.qiime2.org/t/processing-filtering-and-evaluating-the-silva-database-and-other-reference-sequence-data-with-rescript/15494>
...
--i-reads ../04_DADA2_244-244/rep-seqs.qza --p-n-jobs 7 --verbose \
...
Revisar la taxonomía para evaluar cómo ha ido la asignación:
...
No se quedan muchos ASVs sin asignar. La mayoría se asignan a bacterias con asignación a niveles bajos (género) y no hay mucha asignación a grupos no objetivo.

## 06_FILTERING

Sobre los ASVs identificados se aplicarán filtros basados en la abundancia y en la taxonomía.
...
Añadir la taxonomía a la tabla de ASVs, para ver qué hay que eliminar en los filtros de taxonomía:
...
Eliminar en el mismo filtro ASVs asignados a grupos no objetivo (arqueas o eucariotas), ASVs no asignados ("Unassigned"), los asignados a secuencias organulares y aquellos que se quedan a nivel de dominio (d\_\_Bacteria) porque no aportan información.

Revisar cuántos ASVs de este tipo hay:
...
Construir los barplots con QIIME 2 partiendo de la tabla asv-table_no_samples_raref:
...
--m-metadata-file ../../02_METADATA/metadata_visual.txt --o-visualization QIIME2_16S.qzv
...
Representar los barplots en R. Se muestra solo el código para hacer uno de los niveles, pero se representarían igual los 7 niveles. Se guardan como html interactivos.
...
--i-table ../../06_FILTERING/04_rarefaction/asv-table_no_samples_raref.qza \
--p-sampling-depth 8965 --m-metadata-file ../../02_METADATA/metadata_visual.txt \
...
--m-metadata-file ../../02_METADATA/metadata_visual.txt \
